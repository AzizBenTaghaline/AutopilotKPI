...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [usersRes, importsRes, kpisRes, alertsRes, auditRes] =
          await Promise.all([
            apiClient.get("/users"),
  ...
  const configuredThresholds = activeKpis.filter(
    (k) => k.target_value != null && k.direction != null,
  );
    ...
    <div className="admin-dashboard">
      ...
      <section className="stats-grid">
        <StatCard
          ...
          trend={{
            direction: "up",
            text: `${users.length - activeUsers.length} compte(s) inactif(s)`,
          }}
        />
        <StatCard
          ...
          trend={
            lastImport
              ? {
                  direction: "up",
                  text: `${lastImport.nb_lignes_succes} ligne(s) réussie(s)`,
                }
              : null
          }
        />
        <StatCard
          ...
          trend={{
            direction: "up",
            text: `${kpis.length - activeKpis.length} indicateur(s) désactivé(s)`,
          }}
        />
        <StatCard
          ...
          trend={{
            direction: alerts.length > 0 ? "down" : "up",
            text: `${alerts.length} alerte(s) déclenchée(s)`,
          }}
        />
      </section>
      ...
      <section className="admin-main-grid">
        <div className="panel">
          ...
          {auditLogs.length === 0 && (
            <p className="panel-empty">Aucune activité récente.</p>
          )}
          {auditLogs.map((log) => (
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
                  ...
                  <td>
                    <span
                      className={`statut-dot ${u.is_active ? "active" : "inactive"}`}
                    />
