  ...
  return (
    <div className="chef-dashboard">
      <header className="dashboard-header">
        <div>
          ...
          <p className="dashboard-subtitle">
            Chef d'atelier — {new Date().toLocaleDateString("fr-FR", { month: "long", year: "numeric" })}
          </p>
        </div>
      ...
      <section className="chef-main-grid">
        <div className="panel">
          <h2>Retours non clôturés <span className="count-badge">{retoursOuverts.length}</span></h2>
          {retoursOuverts.length === 0 && <p className="panel-empty">Aucun retour en attente.</p>}
          {retoursOuverts.map((r) => (
            <div className="retour-row" key={r.id}>
              <span>{r.cause}</span>
              <span className="retour-date">{new Date(r.date_retour).toLocaleDateString("fr-FR")}</span>
            </div>
          ))}
        </div>

        <div className="panel">
          <h2>Dernières réclamations <span className="count-badge">{reclamations.length}</span></h2>
          {reclamations.length === 0 && <p className="panel-empty">Aucune réclamation.</p>}
              ...
              <tbody>
                {reclamations.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      <span className={`statut-badge statut-${r.statut}`}>{r.statut.replace("_", " ")}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
